...
import { askConfirmation, askRecoveryAction, askAdvancedConfirmation, askInput } from '../utils/prompt.js';
    ...
    try {
      switch (step.action) {
        case 'create_file':
          ...
          actionResult = `✅ Fichier créé: ${path.resolve(step.params.path)}`; // message simple
          break;
        case 'modify_file':
...
async function handleAdvancedConfirmation(actions, options) {
  if (options.auto) {
    console.log(chalk.green('🚀 Mode automatique activé, exécution directe...'));
    return 'execute';
  }
...
export async function agentCommand(task, options = {}) {
  ...
  try {
    ...
    let stepNumber = 0;
    const maxSteps = 20; // Limite de sécurité
    ...
    while (stepNumber < maxSteps) {
      ...
      console.log(chalk.blue(`🤖 Génération de l'action ${stepNumber}...`));
      
      const actionResult = await generateNextAction(task, completedActions, {
        debug: options.debug,
        memory: memory.getContextForTask(task)
      });
    ...
    if (stepNumber >= maxSteps) {
      console.log(chalk.yellow(`\n⚠️  Limite de ${maxSteps} actions atteinte`));
    }
