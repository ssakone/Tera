...
export async function readFileWithLines(params) {
  ...
  // Validation du minimum de 50 lignes seulement si le fichier est assez long
  if (totalLines >= 50 && actualLineDifference < 50) {
    throw new Error(`Minimum 50 lignes requis. Actuellement: ${actualLineDifference} lignes (${startLine}-${actualEndLine}). Ajustez la plage pour avoir au moins 50 lignes.`);
  }
