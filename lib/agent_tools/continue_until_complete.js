...
export async function continueUntilComplete(
  ...
  while (iteration < maxIterations) {
    ...
    console.log(chalk.blue(`\n🔍 Preparation de la prochaine étape...`));
    ...
    if (shouldActuallyContinue) {
      ...
      if (options.auto || await askConfirmation('Exécuter les actions additionnelles ?')) {
  ...
  if (iteration >= maxIterations) {
    console.log(chalk.yellow(`\n⚠️ Nombre maximum d'itérations atteint. Arrêt pour éviter les boucles infinies.`));
  }
