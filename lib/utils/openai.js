...
- inform_user: {"message": "message principal", "type": "info|success|warning|error|complete|tip|comment|update", "title": "titre optionnel", "details": ["détail1", "détail2"], "action_needed": "action recommandée"}
- task_completed: {"task": "description de la tâche terminée", "results": ["résultat1", "résultat2"], "next_step": "prochaine étape suggérée"}
- give_tip: {"tip": "conseil à donner", "context": "contexte optionnel"}
- update_status: {"status": "statut actuel", "progress": "X/Y étapes", "next": "prochaine action"}
- chat: {"response": "réponse conversationnelle", "tone": "friendly|professional|casual|helpful|encouraging|thoughtful", "ask_follow_up": false, "follow_up_question": "question optionnelle"}
</available_actions>
...
- update_status: {"status": "statut actuel", "progress": "X/Y étapes", "next": "prochaine action"}
- chat: {"response": "réponse conversationnelle", "tone": "friendly|professional|casual|helpful|encouraging|thoughtful", "ask_follow_up": false, "follow_up_question": "question optionnelle"}
- greet: {"greeting": "salutation personnalisée", "context": "contexte optionnel"}
- apologize: {"apology": "excuse personnalisée", "clarification": "clarification optionnelle"}
- ask_question: {"question": "question à poser", "context": "contexte optionnel"}
- encourage: {"message": "message d'encouragement", "achievement": "accomplissement à célébrer"}
</available_actions>`;

  const systemPrompt = `Tu es un agent IA expert en développement. Ta précédente réponse JSON a échoué lors du parsing.
...
{
  "analysis": "analyse de l'erreur et correction nécessaire",
  "next_action": {
...
STRUCTURE JSON EXACTE:
{
  ...
  "next_action": {
    "action": "patch_file|create_file|read_file_lines|run_command|create_directory|list_directory|inform_user|task_completed",
    "description": "description précise de l'action",
...
- patch_file: {"path": "test.js", "changes": [{"action": "replace", "line": 5, "old": "const a = 1", "new": "const a = 2"}]} ← MAX 3-4 changements

⚠️ IMPORTANT PATCH_FILE:
- LIMITE à 3-4 changements maximum par action
- Pour plus de modifications, utilise plusieurs actions patch_file successives
- Cela évite les JSON trop longs qui causent des erreurs de parsing
...
Si la tâche est terminée, mets "status": "completed"`;
  ...
  try {
    ...
    // Utiliser la fonction robuste de parsing JSON avec debug
    try {
      const parsed = cleanAndParseJSON(content, { 
        debug: options.debug || false, 
        context: "generateNextAction" 
      });
      ...
      console.log(chalk.red(`❌ Erreur de parsing JSON avancé: ${parseError.message}`));
      console.log(chalk.yellow(`📝 Contenu reçu de Claude:`));
