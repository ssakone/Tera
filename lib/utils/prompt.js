...
export function parseAutoApprovalOptions(autoOption) {
  ...
  if (typeof autoOption === 'string') {
    const specifiedActions = autoOption
      .split(',')
      .map(action => action.trim())
      .filter(action => action.length > 0);
    
    // Actions de communication toujours auto-approuvées (bénignes)
    const alwaysAutoApproved = [
      'chat', 'greet', 'apologize', 'ask_question', 'encourage',
      'inform_user', 'task_completed', 'give_tip', 'update_status'
    ];
    
    const allowedActions = [...new Set([...specifiedActions, ...alwaysAutoApproved])];
    
    console.log(chalk.blue(`🤖 Auto-approbation activée pour: ${chalk.cyan(specifiedActions.join(', '))}`));
    if (alwaysAutoApproved.some(action => !specifiedActions.includes(action))) {
      console.log(chalk.gray(`   + Actions de communication toujours auto-approuvées: ${chalk.cyan(alwaysAutoApproved.join(', '))}`));
    }
